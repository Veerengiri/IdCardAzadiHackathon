...
const celender = require("../models/calender");
const routeruser = express.Router();


routeruser.get("/api/level/:rid",async (req,res)=>{
  try {
    const {level} = await newReg.findById(req.params.rid).select("level -_id");
    res.json({status:'ok',level});
  } catch (er) {
    res.json({status:'error'});
  }
})
routeruser.get("/api/updatemarks/:rid/:marks",async (req,res)=>{
  try {
    const dt = await newReg.findById(req.params.rid);
    if(dt.level==1){
      const mu = await newReg.findByIdAndUpdate(req.params.rid,{test: req.params.marks,level:2});
      res.json({status:"ok"})
    }else{
...
routeruser.post("/api/addcalender",async (req,res)=>{
    try {
      const {uId,date,month,year,message}=req.body;
      const data = await celender.create({
        uId,
        date,
        month,
        year,
...
routeruser.get("/api/celender/:id",async (req,res)=>{
  try {
    const cl = await celender.find({uId:req.params.id});
    res.json({status:"ok",celender:cl});
  } catch (error) {
    res.json({status:"error"});
  }
});
routeruser.delete("/api/celender/:cid",async (req,res)=>{
  try {
    const cl = await celender.findByIdAndDelete(req.params.id);
    if(cl){
      res.json({status:"ok"});
    }else{
